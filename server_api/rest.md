...
推送接口的基础参数是指，所有推送消息的接口的通用参数，切记接口调用参数中必须还要包含 [通用基础参数](###通用基础参数)

具体通用参数见下表：

| 参数名        | 类型   | 必需          | 默认值   | 描述                                                         |
| ------------- | :----- | ------------- | :------- | ------------------------------------------------------------ |
| message       | string | 是            | 无       | 消息体，参见[消息体格式](###消息体格式)                      |
...
| send_time     | string | 否            | 当前时间 | 1.指定推送时间,格式为yyyy-MM-DD HH:MM:SS<br>2.若小于服务器当前时间，则会立即推送<br>3.仅[全量推送](##全量推送)和[标签群推](###标签群推)支持此字段 |
...
**请求参数**：
 ...
 [通用基础参数](###通用基础参数)和[Push API 基础参数](###Push API基础参数)
...
除了[通用基础参数](###通用基础参数)和[Push API 基础参数](###Push API基础参数)，还包括如下特定参数：
...
目前V2版本支持根据消息ID来取消尚未触发的、定时的、[全量推送](###全量推送)或[标签群推](###标签群推)的推送消息
