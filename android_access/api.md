...
|void bindAccount(Context context, String account, XGIOperateCallback callback) |启动并注册APP，同时绑定账号,推荐有帐号体系的APP使用（3.2.2（不包括3.2.2）之前的版本使用）|
...
|void bindAccount(Context context, final String account) |启动并注册APP，同时绑定账号,推荐有帐号体系的APP使用（3.2.2以及3.2.2之后的版本使用，此接口会覆盖设备之前绑定过的账号，仅当前注册的账号生效）|
| void appendAccount(Context context, String account, XGIOperateCallback callback) |启动并注册APP，同时绑定账号,推荐有帐号体系的APP使用（3.2.2以及3.2.2之后的版本使用，此接口保留之前的账号，只做增加操作，一个token下最多只能有3个账号超过限制会自动顶掉之前绑定的账号）|
| void appendAccount(Context context, final String account) |启动并注册APP，同时绑定账号,推荐有帐号体系的APP使用（3.2.2以及3.2.2之后的版本使用，此接口保留之前的账号，只做增加操作，一个token下最多只能有3个账号超过限制会自动顶掉之前绑定的账号）|
| void delAccount(Context context, final String account, XGIOperateCallback callback)  |解绑指定账号（3.2.2以及3.2.2之后的版本使用）|
| void delAccount(Context context, final String account  |解绑指定账号（3.2.2以及3.2.2之后的版本使用）|
